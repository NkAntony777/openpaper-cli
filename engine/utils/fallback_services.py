...
def search_web_dataforseo(
    ...
    with DATAFORSEO_LOCK:
        try:
            ...
            return {"success": True, "results": results[:num_results], "query": query}
...
def scrape_page_with_openpull(
    ...
    async def scrape_with_crawl4ai():
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            if not result.success:
                ...
                elif "404" in err_str or (
                    hasattr(result, "status_code") and result.status_code == 404
                ):
    ...
    try:
        ...
        if result is not None:
            print(
                f"[scrape_page] Result: success={result.get('success')}, content_len={len(result.get('content', ''))}"
            )
            return result
...
def get_url_context_openpull(
    url: str, prompt: str = "", gemini_api_key: Optional[str] = None
) -> Dict[str, Any]:
...
def get_url_context_simple(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print("[simple_scraper] Making request...")
        response = requests.get(url, headers=headers, timeout=15)
        print(
            f"[simple_scraper] Response status: {response.status_code}, content length: {len(response.text)}"
        )
        ...
        if response.status_code == 404:
            ...
            return {
                "success": False,
                "error": f"ERROR: Page not found (404) - URL does not exist: {url}",
                "content": "",
            }
        if response.status_code == 403:
            ...
            return {
                "success": False,
                "error": f"ERROR: Access forbidden (403) - website blocked access: {url}",
                "content": "",
            }
        if response.status_code >= 400:
            ...
            return {
                "success": False,
                "error": f"ERROR: HTTP {response.status_code} - failed to load page: {url}",
                "content": "",
            }
        ...
        print("[simple_scraper] Parsing HTML with BeautifulSoup...")
...
def get_tool_context_with_fallback(
    ...
    urls_to_process = []
    for _key, value in row_data.items():
    ...
    if "web-search" in tools:
        ...
        if any(
            kw in prompt_lower
            for kw in ["today", "current date", "what date", "todays date", "today's date"]
        ):
