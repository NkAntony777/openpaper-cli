...
def add_citations_batch(
    db: CitationDatabase, citations: List[Citation], deduplicate: bool = True, verbose: bool = False
) -> int:
    """
    ...
    existing_keys = {
        (c.authors[0].lower() if c.authors else "", c.year, c.title.lower()) for c in db.citations
    }
