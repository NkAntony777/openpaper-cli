...
def insert_academic_structure(
    docx_path: Path, verbose: bool = False, options: Optional[Dict[str, Any]] = None
) -> bool:
    """
    ...
    try:
        ...
        if cover_end_idx is not None:
            ...
            if verbose:
                print("   ✓ Inserted page break after cover page")
        ...
        if abstract_end_idx is not None:
            ...
            if verbose:
                print("   ✓ Inserted page break after Abstract")
...
def _insert_toc(doc: Document, insert_before_idx: int, verbose: bool = False):
    ...
    for i, para in enumerate(doc.paragraphs):
        if para.text.strip().lower() == "abstract" and "Heading" in (
...
def _insert_metadata_after_date(doc: Document, date_idx: int, options: Dict, verbose: bool):
    ...
    if options.get("project_type"):
        ...
        new_para = _insert_para_after(
            insert_after, options["project_type"].upper(), size=11, small_caps=True
        )
        insert_after = new_para
        additions += 2
    ...
    if options.get("student_id"):
        ...
        insert_after = new_para
        new_para = _insert_para_after(
            insert_after, f"Matriculation No.: {options['student_id']}", size=10
        )
        insert_after = new_para
        additions += 2
    ...
    if options.get("instructor"):
        ...
        insert_after = new_para
        new_para = _insert_para_after(
            insert_after, f"First Supervisor: {options['instructor']}", size=10
        )
        insert_after = new_para
        additions += 2
    ...
    if options.get("second_examiner"):
        new_para = _insert_para_after(
            insert_after, f"Second Examiner: {options['second_examiner']}", size=10
        )
        insert_after = new_para
        additions += 1
