    ...
    def _generate_content_with_grounding(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.session.post(
                url, json=body, headers={"Content-Type": "application/json"}, timeout=self.timeout
            )
                    ...
                    try:
                        ...
                        if available_fallbacks:
                            ...
                            best_key, best_type = bp.get_best_gemini_key(
                                ...
                                fallback_2=available_fallbacks[1]
                                if len(available_fallbacks) > 1
                                else None,
                                fallback_3=available_fallbacks[2]
                                if len(available_fallbacks) > 2
                                else None,
                            )
                            ...
                            if best_key and best_key != self.api_key:
                                safe_print(
                                    f"Gemini API rate limit (429) - switching to {best_type.value}"
                                )
                                ...
                                safe_print(
                                    f"Fallback key ({best_type.value}) also failed: {response.status_code}"
                                )
...
Provide the source title, URL, and a brief snippet explaining relevance."""
    ...
    def _extract_grounding_citations(self, response_data: Dict[str, Any]) -> List[Dict[str, str]]:
        """
                ...
                for chunk in grounding_chunks:
                    ...
                    if web:
                        ...
                        if uri:
                            source["url"] = uri
                            source["title"] = (
                                title if title else uri
                            )  # Use URL as title if title missing
    ...
    def _validate_sources(self, sources: List[Dict[str, str]]) -> List[Dict[str, Any]]:
        """
        ...
        for source in sources:
            ...
            has_doi = bool(
                self._extract_doi_from_academic_url(final_url)
                or self._extract_doi_from_doi_url(final_url)
